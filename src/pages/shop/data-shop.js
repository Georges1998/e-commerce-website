const SHOP_DATA = [
  {
    id: 1,
    title: "Princess Caroline",
    description:
      "A workaholic struggling to balance personal life. She idealizes work, thinking that her hard work can solve anything. She strives for perfectionism and has a deep fear of situations she can’t control. Her struggle to find a balanced relationship and her constant miscarriages shatter her desires to become a mother and add weight to her insecurities of ending up alone.",
    routeName: "princess",
    items: [
      {
        id: 1,
        name: "Happy PC",
        imageUrl:
          "https://medias.spotern.com/spots/w640/155/155396-1533034516.jpg",
        price: 25,
      },
      {
        id: 2,
        name: "Judah and PC",
        imageUrl:
          "https://ca-times.brightspotcdn.com/dims4/default/6782dd1/2147483647/strip/true/crop/3840x2160+0+0/resize/840x473!/quality/90/?url=https%3A%2F%2Fcalifornia-times-brightspot.s3.amazonaws.com%2F76%2F7a%2Fa5541d0f4b53b7a44d06c0bafc80%2Fbojack-horseman-season-6-the-face-of-depression-1589359-00-22-26-08-32312.jpg",
        price: 18,
      },
      {
        id: 3,
        name: "PC and Ruth",
        imageUrl:
          "https://pyxis.nymag.com/v1/imgs/526/536/aefc83097a9c5b56b9addec99f9bda3873-bojack-horseman-06.rsocial.w1200.jpg",
        price: 35,
      },
      {
        id: 4,
        name: "At the office",
        imageUrl: "https://i.redd.it/wy8ftzz0v4m11.jpg",
        price: 25,
      },
      {
        id: 5,
        name: "Dreamy PC",
        imageUrl:
          "https://nofilmschool.com/sites/default/files/styles/facebook/public/bj209_166_1.jpeg?itok=w9Nlm0pu",
        price: 18,
      },
      {
        id: 6,
        name: "100 PCs",
        imageUrl:
          "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/xyzwvtikagyopanbqxlj.png",
        price: 14,
      },
      {
        id: 7,
        name: "PC's Mom",
        imageUrl:
          "https://pyxis.nymag.com/v1/imgs/23a/bdd/ded582984ebd85afbe370da54bc50365f3-bojack-horseman-505.rsocial.w1200.jpg",
        price: 14,
      },
      {
        id: 8,
        name: "Vincent Adultman",
        imageUrl: "https://i.redd.it/rryc99gen1uz.png",
        price: 14,
      },
    ],
  },
  {
    id: 2,
    title: "Todd",
    description:
      "Todd is a guy who is trapped in a man-childlike mind who is completely unaware of his real potential. He is stuck living in people’s couches after he is kicked out of his house. Todd’s journey is one of self-discovery. He longs to understand who he is while trying to find comfort in his friends, who can sometimes not value him as much as he values them.",
    routeName: "Todd",
    items: [
      {
        id: 10,
        name: "All About that Ace App",
        imageUrl:
          "https://www.indiewire.com/wp-content/uploads/2018/09/BoJack_Horseman_S05E05_18m47s27043f.jpg",
        price: 220,
      },
      {
        id: 11,
        name: "Ceo Todd",
        imageUrl:
          "https://static0.srcdn.com/wordpress/wp-content/uploads/2020/09/Todd-Chavez-from-BoJack-Horseman.jpg?q=50&fit=crop&w=740&h=370",
        price: 280,
      },
      {
        id: 12,
        name: "Person of the Year",
        imageUrl:
          "https://static2.srcdn.com/wordpress/wp-content/uploads/2019/10/BoJack-Horseman-Season-6-Todd.jpg?q=50&fit=crop&w=740&h=370",
        price: 110,
      },
      {
        id: 13,
        name: "Todd Show",
        imageUrl:
          "https://carboncostume.com/wordpress/wp-content/uploads/2014/09/toddchavez.jpg",
        price: 160,
      },
      {
        id: 14,
        name: "Todd",
        imageUrl:
          "https://pbs.twimg.com/media/CLmBSndWEAAJFNf.jpg",
        price: 160,
      },
      {
        id: 15,
        name: "Oh no Todd",
        imageUrl: "https://pbs.twimg.com/media/DOCydaRXUAEi_M6.jpg",
        price: 160,
      },
      {
        id: 16,
        name: "Todd and Bojack",
        imageUrl:
          "https://i.pinimg.com/originals/0d/de/37/0dde37114652e2cf5c141d7e5bddc328.jpg",
        price: 190,
      },
      {
        id: 17,
        name: "Todd and his Triangle",
        imageUrl:
          "https://dotandline.net/wp-content/uploads/2017/09/1_m7L7nQlrMixpXuAbWFwYZA.jpeg",
        price: 200,
      },
    ],
  },
  {
    id: 3,
    title: "Diane",
    description:
      "She has a constant existential crisis in which she seeks that her work and her life should mean more than just it. She constantly tries to involve herself in humanitarian services or write relevant articles that in her mind can make a difference, only to find out that maybe most people won’t care about it. She feels broken and unhappy and she is constantly involving herself in situations and relationships that may or may not help her forget.",
    routeName: "Diane",
    items: [
      {
        id: 18,
        name: "Blue Dress Diane",
        imageUrl:
          "https://dotandline.net/wp-content/uploads/2020/03/diane-nguyen-1210x642.jpg",
        price: 125,
      },
      {
        id: 19,
        name: "Diane writing",
        imageUrl:
          "https://variety.com/wp-content/uploads/2020/06/bojack.jpg?w=681&h=383&crop=1",
        price: 90,
      },
      {
        id: 20,
        name: "Diane and Bojack",
        imageUrl:
          "https://cdn.jwplayer.com/v2/media/vnRqf7o2/poster.jpg?width=640",
        price: 90,
      },
      {
        id: 21,
        name: "Diane and MR",
        imageUrl: "https://images2.alphacoders.com/951/thumb-1920-951967.jpg",
        price: 165,
      },
      {
        id: 22,
        name: "Diane's Family",
        imageUrl: "https://i.ytimg.com/vi/7qIcrr27ZJg/maxresdefault.jpg",
        price: 185,
      },
      {
        id: 222,
        name: "Surprised Diane",
        imageUrl:
          "https://64.media.tumblr.com/327f3e4f61a4754fdda15d1c3b235932/tumblr_p12e1pyOMB1ww403yo1_1280.png",
        price: 185,
      },
      {
        id: 242,
        name: "Diane and Bojack after Emmy's award",
        imageUrl: "https://pbs.twimg.com/media/DthjYTLW4AAqoOc.jpg",
        price: 185,
      },
      {
        id: 245,
        name: "Surprised Diane",
        imageUrl:
          "https://64.media.tumblr.com/327f3e4f61a4754fdda15d1c3b235932/tumblr_p12e1pyOMB1ww403yo1_1280.png",
        price: 185,
      },
    ],
  },
  {
    id: 4,
    title: "bojack horseman",
    description:
      "He is a struggling addict that lives completely haunted by his past. With this character, this show satirizes and deconstructs the toxic male antihero trope. He starts as a plain asshole who could care less about how his actions affect other people. He lives a state of narcissism where he is shielded by his fame and never is held accountable for his destructive behavior.",
    routeName: "bojack",
    items: [
      {
        id: 23,
        name: "Season 1 Bojack",
        imageUrl: "https://scotscoop.com/wp-content/uploads/2020/04/bojack.jpg",
        price: 25,
      },
      {
        id: 24,
        name: "Gray Hair Bojack",
        imageUrl:
          "https://static.highsnobiety.com/thumbor/tdUeFdQZ-PMt1Ty9JfCGtWDLMhQ=/1200x720/static.highsnobiety.com/wp-content/uploads/2020/01/31122103/bojack-horseman-finale-twitter-reactions-00.jpg",
        price: 20,
      },
      {
        id: 25,
        name: "Philbert Bojack",
        imageUrl:
          "https://www.indiewire.com/wp-content/uploads/2018/09/BoJack_Horseman_S05E01_3m37s5212f1.jpg",
        price: 80,
      },
      {
        id: 26,
        name: "Sarah lynn",
        imageUrl:
          "https://www.gannett-cdn.com/-mm-/82c3f6063a606c86ef1574015ba5567f5ba03b12/c=10-0-4477-2524/local/-/media/2016/07/20/USATODAY/USATODAY/636046197709872604-BoJack-Horseman-S03E11-21m39s2598f.JPG",
        price: 80,
      },
      {
        id: 27,
        name: "Rooftop",
        imageUrl:
          "https://tvline.com/wp-content/uploads/2020/01/bojack-horseman-season-6-episode-16-series-finale.png?w=620",
        price: 45,
      },
      {
        id: 28,
        name: "Professor Horseman",
        imageUrl:
          "https://i.guim.co.uk/img/media/f41465fa8789c86ca5acf4a0f48e8e236f298f11/60_0_1799_1080/master/1799.jpg?width=1200&height=1200&quality=85&auto=format&fit=crop&s=6879b498f5b65641d03605cc333ea33a",
        price: 135,
      },
      {
        id: 29,
        name: "Funeral",
        imageUrl:
          "https://www.rollingstone.com/wp-content/uploads/2018/09/bojack-horseman-spoilers.jpg",
        price: 20,
      },
      {
        id: 129,
        name: "Nice Bojack",
        imageUrl:
          "https://www.indiewire.com/wp-content/uploads/2020/01/Screen-Shot-2020-01-15-at-8.51.52-AM.png?w=780",
        price: 120,
      },
    ],
  },
  {
    id: 5,
    title: "Mr. Peanutbutter",
    description:
      "He is a man that refuses to grow up into maturity by living in blissful ignorance where he copes with his day to day problems with unbreakable optimism. He always has faith that destiny will eventually solve his reality.",
    routeName: "mr",
    items: [
      {
        id: 30,
        name: "Senator Mr.B",
        imageUrl:
          "https://cdn.flickeringmyth.com/wp-content/uploads/2017/09/bojack-season-4.jpg",
        price: 325,
      },
      {
        id: 31,
        name: "Sad Dog",
        imageUrl:
          "https://am21.mediaite.com/tms/cnt/uploads/2019/07/How-BoJack-Horsemans-Mr.-Peanutbetter-Exposes-the-Problem-with-Nice-Guys.jpg",
        price: 20,
      },
      {
        id: 32,
        name: "Doggie Doggie What Now?",
        imageUrl:
          "https://www.indiewire.com/wp-content/uploads/2017/09/bj212_184.jpeg",
        price: 25,
      },
      {
        id: 33,
        name: "Diane and Mr",
        imageUrl:
          "https://dotandline.net/wp-content/uploads/2017/09/1_TfEEDuf8Ivky-pec7cmL1g.jpeg",
        price: 25,
      },
      {
        id: 34,
        name: "Dinner",
        imageUrl:
          "https://c4.wallpaperflare.com/wallpaper/177/519/617/tv-show-bojack-horseman-mr-peanutbutter-bojack-horseman-princess-carolyn-wallpaper-preview.jpg",
        price: 40,
      },
      {
        id: 35,
        name: "Skiing",
        imageUrl:
          "https://pictures.betaseries.com/banners/episodes/282254/5689236.jpg",
        price: 25,
      },
      {
        id: 35,
        name: "Burgundy T-shirt",
        imageUrl: "https://images3.alphacoders.com/106/1064490.jpg",
        price: 25,
      },
      {
        id: 135,
        name: "Hollywoo Emmy's",
        imageUrl:
          "https://pyxis.nymag.com/v1/imgs/4e7/d04/272d91377ec54301f73325a2573090faa4-21-bojack-10.rsquare.w700.jpg",
        price: 25,
      },
    ],
  },
];

export default SHOP_DATA;
